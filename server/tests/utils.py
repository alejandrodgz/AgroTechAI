...
def mock_requests_session(response_data: Dict[str, Any], status_code: int = 200):
    ...
    session.post.return_value.json.return_value = create_mock_ollama_response(
        response_data
    )
    return session
    ...
    return {
        "image_vision": {
            ...
            "environmental_context": (
                "Signos de estrés hídrico y posible deficiencia nutricional"
            ),
...
def assert_agent_response_structure(response: Dict[str, Any], agent_type: str):
    ...
    if agent_type == "agri_vision":
        ...
        assert response["leaf_condition"] in [
            "excellent",
            "good",
            "fair",
            "poor",
    ...
    if agent_type == "crop_master":
        ...
        assert response["risk_assessment"] in [
            "low",
            "medium",
            "high",
            "critical",
